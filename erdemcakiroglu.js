...
\`>\` <@&898447333052805131> Rolü Almak İçin Etkinlik Katılımcısı! Butonuna.
...
client.on('clickButton', async (button) => {
    ...
    if (button.id === 'EtkinlikKatılımcısı') {
      ...
      if (button.id === 'DuyuruKatılımcısı') {
        if (button.clicker.member.roles.cache.get((ayarlar.DuyuruKatılımcısı))) {
            ...
            await button.reply.think(true);
            await button.reply.edit("Duyuru Katılımcısı rolü başarıyla üzerinizden alındı!")
        } else {
            await button.clicker.member.roles.add(((ayarlar.DuyuruKatılımcısı)))
            await button.reply.think(true);
            await button.reply.edit("Duyuru Katılımcısı rolünü başarıyla aldınız!")
        }
    }
